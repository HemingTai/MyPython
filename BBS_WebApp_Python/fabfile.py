...
env.user = 'ubuntu'
env.sudo_user = 'root'
env.hosts = ['111.231.134.142']
env.password = 'daihuiming112911'

db_user = 'root'
db_password = '99112911'
...
def deploy():
    ...
    # put('dist/%s' % _TAR_FILE, _REMOTE_TMP_TAR, use_sudo=True)
    ...
    #     sudo('chown -R ubuntu %s' % newdir)
    # with settings(warn_only=True):
    #     sudo('supervisorctl stop awesome')
    #     sudo('supervisorctl start awesome')
    #     sudo('/etc/init.d/nginx reload')
    # put('conf/supervisor/awesome.conf','/etc/supervisor/conf.d/awesome.conf', use_sudo=True)
    put('conf/nginx/awesome','/etc/nginx/sites-available/', use_sudo=True)
